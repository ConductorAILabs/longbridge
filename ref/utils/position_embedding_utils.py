...
def compute_temporal_freqs(
    ...
    # Mac/MPS bridge: MPS doesn't support fp64. Use fp32 on MPS (sufficient
    # precision for RoPE angle math at our sequence lengths).
    hi_dtype = torch.float32 if device.type == 'mps' else torch.float64
    base_angles = torch.angle(freqs_t[1]).to(hi_dtype)
    positions = torch.arange(f, device=device, dtype=hi_dtype) + start_frame
    if torch.is_tensor(temporal_offset):
        offset = temporal_offset.to(device=device, dtype=hi_dtype)
        if offset.ndim == 0:
