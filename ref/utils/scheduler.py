...
class SchedulerInterface(ABC):
    ...
    def convert_x0_to_noise(
        ...
        # Mac/MPS bridge: MPS doesn't support fp64.
        hi_dtype = torch.float32 if x0.device.type == 'mps' else torch.float64
        x0, xt, alphas_cumprod = map(
            lambda x: x.to(hi_dtype).to(x0.device), [x0, xt,
                                                 self.alphas_cumprod]
        )
    ...
    def convert_noise_to_x0(
        ...
        # Mac/MPS bridge: MPS doesn't support fp64.
        hi_dtype = torch.float32 if noise.device.type == 'mps' else torch.float64
        noise, xt, alphas_cumprod = map(
            lambda x: x.to(hi_dtype).to(noise.device), [noise, xt,
                                                    self.alphas_cumprod]
        )
        alpha_prod_t = alphas_cumprod[timestep].reshape(-1, 1, 1, 1)
        beta_prod_t = 1 - alpha_prod_t
    ...
    def convert_velocity_to_x0(
        ...
        # Mac/MPS bridge: MPS doesn't support fp64.
        hi_dtype = torch.float32 if velocity.device.type == 'mps' else torch.float64
        velocity, xt, alphas_cumprod = map(
            lambda x: x.to(hi_dtype).to(velocity.device), [velocity, xt,
                                                       self.alphas_cumprod]
        )
        alpha_prod_t = alphas_cumprod[timestep].reshape(-1, 1, 1, 1)
        beta_prod_t = 1 - alpha_prod_t
