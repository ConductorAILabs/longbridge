        ...
        **conditional_dict: dict
        ...
        if exit_step == num_denoising_steps - 1:
            ...
            denoised_timestep_from = 1000 - torch.argmin(
                (self.scheduler.timesteps.to(_d) - unipc_timesteps[exit_step].to(_d)).abs(), dim=0).item()
        else:
            denoised_timestep_to = 1000 - torch.argmin(
                (self.scheduler.timesteps.to(_d) - unipc_timesteps[exit_step + 1].to(_d)).abs(), dim=0).item()
            denoised_timestep_from = 1000 - torch.argmin(
                (self.scheduler.timesteps.to(_d) - unipc_timesteps[exit_step].to(_d)).abs(), dim=0).item()
