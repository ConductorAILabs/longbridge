...
def causal_rope_apply(x, grid_sizes, freqs, start_frame=0, t_scale=1.0,
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        if is_mps:
            # Treat last dim of x as (real, imag) pairs.
            x_pair = x[i, :seq_len].to(torch.float32).reshape(seq_len, n, -1, 2)
            x_re, x_im = x_pair[..., 0], x_pair[..., 1]
            # Build cos/sin from complex freqs.
            t_re = temporal_freqs.real.to(torch.float32).view(f, 1, 1, -1).expand(f, h, w, -1)
            t_im = temporal_freqs.imag.to(torch.float32).view(f, 1, 1, -1).expand(f, h, w, -1)
            h_re = freqs[1][:h].real.to(torch.float32).view(1, h, 1, -1).expand(f, h, w, -1)
            h_im = freqs[1][:h].imag.to(torch.float32).view(1, h, 1, -1).expand(f, h, w, -1)
            w_re = freqs[2][:w].real.to(torch.float32).view(1, 1, w, -1).expand(f, h, w, -1)
            w_im = freqs[2][:w].imag.to(torch.float32).view(1, 1, w, -1).expand(f, h, w, -1)
            cos = torch.cat([t_re, h_re, w_re], dim=-1).reshape(seq_len, 1, -1)
            sin = torch.cat([t_im, h_im, w_im], dim=-1).reshape(seq_len, 1, -1)
            # (a+ib)(c+id) = (ac-bd) + i(ad+bc)
            o_re = x_re * cos - x_im * sin
            o_im = x_re * sin + x_im * cos
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_inference(
        ...
        if self.freqs.device != device:
            # Mac/MPS bridge: MPS doesn't support fp64/complex128.
            # Downcast freqs before moving to MPS.
            if device.type == 'mps':
                if self.freqs.dtype == torch.complex128:
                    self.freqs = self.freqs.to(torch.complex64)
                elif self.freqs.dtype == torch.float64:
                    self.freqs = self.freqs.to(torch.float32)
